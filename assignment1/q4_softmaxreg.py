...
def getSentenceFeature(tokens, wordVectors, sentence):
    ...
    for word in sentence:
        try:
            sentVector = sentVector + wordVectors[tokens[word.encode()],:]
...
def softmaxRegression(features, labels, weights, regularization = 0.0, nopredictions = False):
    ...
    num_labels = np.shape(weights)[1]

    d = prob - np.eye(num_labels)[labels]
    grad = (features.T@d / N) + regularization*weights
