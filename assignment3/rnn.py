...
class RNN_Model():
    ...
    def add_model_vars(self):
        ...
        with tf.variable_scope('Composition'):
            ...
            self.W1 = tf.get_variable("W1", shape=(2*self.config.embed_size,
                                                   self.config.embed_size))
            self.b1 = tf.get_variable("b1", shape=(1, self.config.embed_size))
            ### END YOUR CODE
        with tf.variable_scope('Projection'):
            ### YOUR CODE HERE
            self.U = tf.get_variable("U", shape = (self.config.embed_size,
                                                   self.config.label_size))
            self.bs = tf.get_variable("bs", shape = (1,
                                                     self.config.label_size))
            ### END YOUR CODE
    ...
    def add_model(self, node):
        ...
        if node.isLeaf:
            ### YOUR CODE HERE
            lookup = tf.gather(self.embedding, self.vocab.encode(node.word))
            ...
            ### YOUR CODE HERE
            h_new = tf.concat([node_tensors[node.left],
                                  node_tensors[node.right]], 1)
            curr_node_tensor = tf.nn.relu(tf.matmul(h_new, self.W1) + self.b1)
            ### END YOUR CODE
    ...
    def loss(self, logits, labels):
        ...
        with tf.variable_scope("Projection", reuse=True):
            self.U = tf.get_variable("U")
        loss = self.config.l2 * (tf.nn.l2_loss(self.W1) + tf.nn.l2_loss(self.U))
        loss += \
        tf.reduce_sum(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits,
    ...
    def train(self, verbose=True):
        ...
        for epoch in range(self.config.max_epochs):
            ...
            if val_loss < best_val_loss:
                 import glob
                 file_name =\
                 glob.glob("weights/%s.temp*"%self.config.model_name)[0]
                 shutil.copyfile(file_name, './weights/%s'%self.config.model_name)
