...
class NERModel(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ### YOUR CODE HERE
      embeddings = tf.get_variable("L", [len(self.wv), self.config.embed_size])
      embed = tf.nn.embedding_lookup(embeddings, self.input_placeholder) 
      ### END YOUR CODE
      return tf.reshape(embed, [-1, self.config.window_size*self.config.embed_size]) 
  ...
  def add_loss_op(self, y):
    ...
    """
    ### YOUR CODE HERE
    loss =tf.nn.softmax_cross_entropy_with_logits(logits=y, labels=self.labels_placeholder)
    loss = tf.reduce_mean(loss)
    loss = loss + tf.add_n(tf.get_collection("loss")) * (self.config.l2/2)
    ### END YOUR CODE
