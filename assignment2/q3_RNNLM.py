...
class RNNLM_Model(LanguageModel):
  ...
  def add_placeholders(self):
    ...
    self.labels_placeholder = tf.placeholder(tf.int32, shape=(None,
                                                                self.config.num_steps),
                                            name="labels")
    self.dropout_placeholder = tf.placeholder(tf.float32, name="drop_out")
    ### END YOUR CODE
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ...
      inputs = [tf.squeeze(x) for x in tf.split(embed, axis=1,
                                                num_or_size_splits=self.config.num_steps)]   # not sure
      ### END YOUR CODE
      return inputs
  ...
  def add_projection(self, rnn_outputs):
    ...
    with tf.variable_scope("projection"):
        ...
        outputs = [tf.matmul(r, U) + b_2 for r in rnn_outputs] 
    ### END YOUR CODE
    return outputs
  ...
  def add_loss_op(self, output):
    ...
    loss = sequence_loss(logits=[output],
  ...
  def add_model(self, inputs):
    ...
    with tf.variable_scope("RNN_LM"):
        ...
        I = tf.get_variable("I", shape=(self.config.embed_size,
                                        self.config.hidden_size))
    ...
    with tf.variable_scope("RNN_LM") as scope:
        ...
        self.initial_state = tf.zeros(shape=(self.config.batch_size,
                                             self.config.hidden_size))
...
def generate_text(session, model, config, starting_text='<eos>',
  ...
  for i in range(stop_length):
    ### YOUR CODE HERE
    feed_dict = {model.input_placeholder: [[tokens[-1]]], model.dropout_placeholder:1,
